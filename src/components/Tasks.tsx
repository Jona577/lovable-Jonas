...
const Tasks: React.FC<TasksProps> = ({ isDarkMode, onViewChange }) => {
  ...
  return (
    <div className="animate-fadeIn">
      ...
      {view !== 'add_task_form' && (
        <>
          ...
          <div className="flex justify-between items-start mb-2">
            <div>
              <h2 className="text-2xl sm:text-4xl font-black text-slate-800 tracking-tight">Tarefas</h2>
              <p className="text-slate-400 font-medium text-xs sm:text-sm">Gerencie sua rotina diária com facilidade.</p>
            </div>
          </div>

          <div className="flex flex-col md:flex-row justify-between items-center mt-4 sm:mt-6 mb-6 sm:mb-10 gap-4 sm:gap-6">
            <div className="bg-white border-2 border-black p-0.5 rounded-2xl flex shadow-sm w-full sm:w-auto">
              <button
                ...
                className={`py-2 px-4 sm:px-8 rounded-xl font-bold text-xs sm:text-sm transition-all active:scale-95 flex-1 sm:flex-none ${timeframe === 'today' && view === 'tasks' ? 'bg-black text-white' : 'text-black hover:bg-slate-50'}`}
              >
                Para hoje
              </button>
              <button
                onClick={() => { setTimeframe('other'); setView('tasks'); }}
                className={`py-2 px-4 sm:px-8 rounded-xl font-bold text-xs sm:text-sm transition-all active:scale-95 flex-1 sm:flex-none ${timeframe === 'other' && view === 'tasks' ? 'bg-black text-white' : 'text-black hover:bg-slate-50'}`}
              >
                Para outro dia
              </button>
            </div>
      ...
      <div className="bg-white rounded-[1.5rem] sm:rounded-[2rem] border-[1.5px] border-[#4A69A2] p-4 sm:p-8 shadow-sm min-h-[400px] flex flex-col overflow-hidden">
        {view === 'tasks' && (
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {categories.map(cat => (
                  <div key={cat.id} className="relative group">
                    ...
                    <div className="absolute top-2 right-2 flex gap-2 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-all scale-100 md:scale-90 md:group-hover:scale-100">
                      <button
