...
const Sidebar: React.FC<SidebarProps> = ({ activePage, onNavigate, userName, isDarkMode, isMobileOpen, onCloseMobile, isHidden }) => {
  ...
  return (
    <>
      ...
      <nav className={`
        ${brandBg} flex flex-col h-full z-50
        ...
        /* Desktop: static sidebar, transitions width and opacity */
        md:static md:translate-x-0 md:shrink-0 md:overflow-hidden
        ${isHidden
          ? 'md:w-0 md:opacity-0 md:pointer-events-none'
          : isCollapsed ? 'md:w-20 md:opacity-100' : 'md:w-72 md:opacity-100'
        }
      `}>
        {/* Inner container to prevent content squishing during width transition */}
        <div className={`${isCollapsed ? 'w-20' : 'w-72'} h-full flex flex-col shrink-0`}>
