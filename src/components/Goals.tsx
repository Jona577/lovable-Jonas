  ...
  const renderPeriodButton = (period: GoalPeriod) => {
    ...
    return (
      <button
        ...
        className={`flex-1 min-w-0 px-1 py-2 sm:px-3 sm:py-3 rounded-lg sm:rounded-xl text-[9px] sm:text-sm font-black transition-all duration-300 whitespace-nowrap border-2 overflow-hidden text-ellipsis ${isActive
    ...
    <div className="space-y-6 md:space-y-10 animate-fadeIn">
      ...
      {view === 'goals' && (
        <div className="space-y-6 md:space-y-10 animate-fadeIn">
          ...
          <div className={`rounded-xl sm:rounded-2xl border-2 border-black p-1 sm:p-2 overflow-x-auto ${isDarkMode ? 'bg-slate-900' : 'bg-white'}`}>
            <div className="flex min-w-max sm:min-w-0 w-full gap-1 sm:gap-1.5 mb-1 sm:mb-1.5">
              {row1.map(renderPeriodButton)}
            </div>
            <div className="flex min-w-max sm:min-w-0 w-full gap-1 sm:gap-1.5">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
              {categories.map((category) => (
                ...
                >
                  ...
                  <div className={`absolute inset-0 flex items-start justify-start gap-1 p-2 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity rounded-2xl pointer-events-none`}>
                    {/* Edit button */}
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
