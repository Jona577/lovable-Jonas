...
const Habits: React.FC<HabitsProps> = ({ isDarkMode }) => {
  ...
  const renderTimerModal = () => {
    ...
    return (
      <div className="fixed inset-0 z-[3000] flex flex-col items-center justify-center p-4 animate-fadeIn transition-colors duration-500" style={{ backgroundColor: habitColor }}>
        ...
        <div className="text-center max-w-3xl w-full relative z-10 flex flex-col items-center justify-center h-full gap-8 sm:gap-16">

          <div className="relative w-[220px] h-[220px] sm:w-[340px] sm:h-[340px] md:w-[450px] md:h-[450px] flex items-center justify-center bg-transparent rounded-full">
            ...
            <div className="flex flex-col items-center gap-1 sm:gap-2 z-10 px-2 text-center">
              <span className={`text-[9px] sm:text-[12px] font-black uppercase tracking-wider mb-1 sm:mb-2 ${contentColorClass} opacity-60`}>Tempo Restante</span>
              <div className={`font-black text-4xl sm:text-6xl md:text-8xl font-mono tabular-nums tracking-tighter ${contentColorClass}`}>
                {formatTime(timeLeft)}
              </div>
              <div className={`mt-2 sm:mt-4 px-3 sm:px-6 py-1 sm:py-2 rounded-full text-[9px] sm:text-[12px] font-black uppercase tracking-wide border-2 ${isTimerPaused ? 'opacity-40 border-transparent' : 'animate-pulse border-white/20'} ${contentColorClass}`}>
                {isTimerPaused ? 'Pausado' : 'Em progresso'}
              </div>
            </div>
          </div>

          <div className="flex flex-col items-center gap-5 sm:gap-8 w-full max-w-sm">
            ...
            <div className="w-full px-6">
              <button
                ...
                className={`w-full py-3 sm:py-4 font-black text-[10px] uppercase tracking-widest border-2 rounded-2xl transition-all ${isBlackBg ? 'text-white border-white' : 'text-black border-slate-300 hover:bg-black/5'}`}
              >
                Finalizar Agora
              </button>
            </div>
        ...
        <div className="bg-white p-10 rounded-[2.5rem] shadow-2xl max-w-lg w-full border-[3px] border-black">
          ...
          <div className="grid grid-cols-3 gap-2 sm:gap-4 mb-8">
            {([
              ...
              return (
                <button key={opt.label} onClick={() => setSelectedDifficulty(opt.label)} className={`flex flex-col items-center gap-2 sm:gap-3 p-3 sm:p-5 border-2 border-black rounded-2xl transition-all group ${isActive ? opt.color + ' text-white' : 'bg-white text-slate-800 ' + opt.hover + ' hover:text-white'}`}>
  ...
  const renderCustomForm = () => {
    ...
    return (
      <div className="bg-white border-2 border-black rounded-3xl p-6 sm:p-10 w-full max-w-4xl mx-auto shadow-2xl animate-fadeIn">
  ...
  return (
    <div className="min-h-screen">
      ...
      {isTabbedView ? (
        <div className="animate-fadeIn">
          <header className="mb-6 sm:mb-12"><h2 className={`text-3xl sm:text-5xl font-black ${textColor} tracking-tight`}>Meus Hábitos</h2><p className="text-slate-500 mt-2 sm:mt-3 font-medium text-sm sm:text-lg">Gerencie sua evolução constante.</p></header>
          <div className="flex flex-col md:flex-row justify-between items-center mb-6 sm:mb-10 gap-4 sm:gap-6"><div className="bg-white border-2 border-black p-0.5 rounded-2xl flex shadow-sm w-full sm:w-auto"><button onClick={() => setView('list')} className={`py-2 px-6 sm:px-8 rounded-xl font-bold text-sm transition-all flex-1 sm:flex-none ${view === 'list' ? 'bg-black text-white' : 'text-black hover:bg-slate-50'}`}>Meus hábitos</button></div><div className="flex flex-wrap items-center gap-2 sm:gap-4 w-full sm:w-auto"><button onClick={() => setView('categories')} className={`flex-1 sm:flex-none px-4 sm:px-8 py-2.5 rounded-xl border-2 border-[#A855F7] font-bold text-xs sm:text-sm transition-all shadow-sm ${view === 'categories' ? 'bg-[#A855F7] text-white' : 'text-[#A855F7] bg-white'}`}>Minhas Categorias</button><button onClick={() => setView('create_category')} className={`flex-1 sm:flex-none px-4 sm:px-8 py-2.5 rounded-xl border-2 border-[#3B82F6] font-bold text-xs sm:text-sm transition-all shadow-sm ${view === 'create_category' ? 'bg-[#3B82F6] text-white' : 'text-[#3B82F6] bg-white'}`}>Criar Categoria</button></div></div>
