 ...
 *
 * Configuração:
 *   VITE_GROQ_API_KEY=sua_chave_aqui  (no arquivo .env)
 *
 * Uso:
...
if (!import.meta.env.VITE_GROQ_API_KEY) {
  console.error(
    ...
    'nas variáveis de ambiente antes de usar as funcionalidades de IA.'
  );
}
...
// ---------------------------------------------------------------------------
// Função principal — envia requisição para a API
// ---------------------------------------------------------------------------
/**
...
export async function generateAIResponse(
  ...
  if (!GROQ_API_KEY) {
    const msg =
      'VITE_GROQ_API_KEY não configurada. Adicione a chave Groq ao seu arquivo .env.';
